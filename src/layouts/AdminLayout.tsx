  ...
  const menuItems = [
    {
      ...
      label: <Link to="/admin/revenue" className="text-[14px] font-medium transition-colors">Revenue Center</Link>,
    },
    {
      key: '/admin/geospatial',
      icon: <Map size={22} />,
      label: <Link to="/admin/geospatial" className="text-[14px] font-medium transition-colors">Geospatial Intel</Link>,
    },
    {
      key: '/admin/operators',
    ...
    },
    {
      ...
      label: <Link to="/admin/support" className="text-[14px] font-medium transition-colors">Support</Link>,
    },
  ]
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-[#050811] border-r-2 border-emerald-500/10 relative overflow-hidden">
      ...
      <div className="p-6 border-t border-white/5 bg-black/20 relative z-10">
        ...
        >
          <LogOut size={18} />
          <span className="text-sm font-black uppercase tracking-widest text-shadow-sm">Sign Out</span>
        </button>
      </div>
    </div>
  )
